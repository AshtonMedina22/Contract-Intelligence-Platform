...
async function ProposalsWorkspaces() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) return <p className="text-sm">Sign in to view proposal workspaces.</p>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">Proposal workspaces</h1>
          <p className="max-w-2xl text-sm text-muted-foreground">
            One workspace per pursuit — requirements, verified pricing, cost planning, competitor evidence, and
            contract linkage. Canonical data still comes only from human-verified promotion.
          </p>
        </div>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-medium">Active pursuits ({active.length})</h2>
        {active.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No active workspaces. Upload a solicitation via Intake — identity promotion creates an opportunity
            you can open here.
          </p>
        ) : (
...
export default function ProposalsPage() {
  return (
    <Suspense fallback={<p className="text-sm text-muted-foreground">Loading workspaces…</p>}>
      <ProposalsWorkspaces />
    </Suspense>
  );
}
